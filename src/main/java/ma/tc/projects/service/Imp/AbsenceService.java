...
public class AbsenceService implements ICrudService<Absence, Long> {
	...
	public List<Absence> getMonthAbsence(long idOuvrier, int month) {
		if (month < 1)
			throw new RuntimeException("Fail! -> Cause: month number have to be Absolutely positive.");
		if (month > 12)
			throw new RuntimeException("Fail! -> Cause: month number must be less than 12");

		return absenceRepo.getMonthAbsence(idOuvrier, month);
	}

	public void deleteByOuvrier(long idOuvrier) {
		absenceRepo.deleteByOuvrier(idOuvrier);
	}
